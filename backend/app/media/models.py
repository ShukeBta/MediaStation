...
class MediaChapter(TimestampMixin, Base):
    ...
    end_time: Mapped[float | None] = mapped_column(Float, nullable=True)  # 秒，可为空（最后一章）
    description: Mapped[str | None] = mapped_column(Text, nullable=True)
    thumbnail_url: Mapped[str | None] = mapped_column(String(1000), nullable=True)
    source: Mapped[str] = mapped_column(String(20), default="ai")       # ai / manual
    confidence: Mapped[float] = mapped_column(Float, default=1.0)       # AI 置信度 0-1

    # 关系
    media_item: Mapped[MediaItem] = relationship()


class MediaHighlight(TimestampMixin, Base):
    """媒体精彩片段（AI 提取或手动标注）"""
    __tablename__ = "media_highlights"
    ...
    score: Mapped[float] = mapped_column(Float, default=0.0)           # 精彩度评分 0-10
    tags: Mapped[str | None] = mapped_column(Text, nullable=True)      # JSON: ["action", "emotional"]
    source: Mapped[str] = mapped_column(String(20), default="ai")      # ai / manual
    thumbnail_url: Mapped[str | None] = mapped_column(String(1000), nullable=True)
...
class MediaCoverCandidate(TimestampMixin, Base):
    """媒体封面候选（AI 生成）"""
    __tablename__ = "media_cover_candidates"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    media_item_id: Mapped[int] = mapped_column(ForeignKey("media_items.id", ondelete="CASCADE"), nullable=False, index=True)
    url: Mapped[str] = mapped_column(String(1000), nullable=False)      # 封面图片 URL 或路径
    timestamp: Mapped[float | None] = mapped_column(Float, nullable=True)  # 截帧时间戳（秒）
    score: Mapped[float] = mapped_column(Float, default=0.0)            # 质量评分 0-10
    is_selected: Mapped[bool] = mapped_column(Boolean, default=False)   # 是否为当前选中封面
    source: Mapped[str] = mapped_column(String(20), default="ai")       # ai / manual / tmdb
